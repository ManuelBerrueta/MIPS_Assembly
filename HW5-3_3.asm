...
Quicksort:
	...
	partitionLowerLoop:
		...
		slt     $t6, $t2, $t5		# myArray[i] <= 
		bne     $t7, $zero, partitionHighLoop

		addi    $t3, $t3, 1			# i++
		slt     $t6, $a2, $t3		# j < i
		bne		$t6, $zero, partitionHighLoop	# if i > j jump
	...
	partitionHighLoop:
		...
		slt     $t7, $t7, $t2
		bne     $t7, $zero, endPartitionLoop

		addi    $t4, $t4, -1			# j--
		slt     $t7, $a1, $t4			# i < j
		beq		$t7, $zero, endPartitionLoop	# if i >= j exit loop


	endPartitionLoop:
		slt     $t7, $t4, $t3
		bne		$t7, $zero, endPartition	# if $t7 != zerot jump to EndPartition

		# new temps to keep track
		swap:
			sll     $t5, $t3, 2			# i * 4 bytes
			add		$t5, $a0, $t4		# myArray[i] address
